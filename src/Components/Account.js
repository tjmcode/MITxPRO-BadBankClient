...
function Account()
{
    ...
    function checkFields()
    {
        ...
        if (parseFloat(balance) < MINIMUM_OPENING_DEPOSIT) return false;
    ...
    function createAccount_Click()
    {
        ...
        if (!checkFields())
        {
            console.log(`Create Account: Failed vaildation, no Account created.`);
            return;
        }

        /* -- no-DB Version, internal App Data Store...
        ctx.Users.push({name, email, password, balance});
        */

        // -- MongoDB (thru a DAL) version...
        const route = `/account/create/${name}/${email}/${password}/${balance}`;
        ...
        // Create Account Card is now in 'Show Status' mode...
        setNeedInput(false);
    }
    ...
    // -- MongoDB (thru a DAL) version... get Users into local App Data Store for buildAccountList()...
    //    NOTE: These is called once on Page Load by React.
    React.useEffect(() =>
    {
        const route = `/account/all`;

        // get all Account data from Server and hold in the App's Context...
        fetch(route)
            .then(response => response.json())
            .then(serverAccounts =>
            {
                ctx.setUsers(serverAccounts);
